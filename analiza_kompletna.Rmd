...
Pre nego što nastavimo sa popunjavanjem preostalih nedostajućih vrednosti (Likes, Comments, Stream) i sa feature engineering-om (posebno Artist_LOO_stream), moramo prvo da podelimo podatke na trening i test skup. Razlog za ovo je sprečavanje **data leakage-a** (curenja podataka): koraci koji slede računaju statistike (medijane, regresione modele) preko grupa redova - po izvođaču, po opsegu Views-a - a ako bismo te statistike računali na celom skupu pre podele, test skup bi delimično "video" informaciju o samom sebi kroz te statistike, čak i kada nijedan pojedinačni red direktno ne koristi sopstvenu vrednost. Konkretno, ako bi se npr. medijana Stream-a po izvođaču računala na celom skupu, a zatim se ta ista podela slala i u trening i u test, test pesme bi uticale na vrednost feature-a koji se koristi za predikciju trening pesama (i obrnuto), što bi dalo veštački optimistične metrike modela.

Zbog toga podelu radimo ovde, odmah nakon strukturnog čišćenja (has_youtube indikator, popunjavanje praznih YouTube kolona, Description), a pre bilo kakve imputacije ili enkodiranja koje computes statistiku preko više redova. Od ovog trenutka, svaka takva statistika će se **fitovati isključivo na trening skupu**, a zatim primeniti (mapirati) i na trening i na test skup - test skup nikada neće uticati na vrednost bilo kog feature-a, čak ni indirektno.
...
```
Za obe kolone kao najbolja metoda pokazala se regresija na osnovu views, tako da ćemo koristiti tu metodu. Sada je bitno da model **fitujemo samo na trening skupu**, a zatim isti (zamrznuti) model primenimo i na trening i na test skup - test skup ni u jednom trenutku ne učestvuje u određivanju koeficijenata regresije.
...
```
Za obe vrste podataka, najbolje je koristiti imputaciju na osnovu Artist-a za grupu bez YouTube videa, dok se za grupu sa YouTube videom najbolje pokazala medijana po Views-decilima. Sada svaku od ovih statistika računamo **isključivo na trening skupu**, i tako zamrznutu je primenjujemo i na trening i na test.
...
```{r}
faktorizuj_strukturu <- function(df) {
  df %>%
    mutate(
...
train <- faktorizuj_strukturu(train)
test <- faktorizuj_strukturu(test)
```
...
Sam LOO princip (izuzimanje trenutne pesme) rešava curenje na nivou pojedinačnog reda, ali ne i curenje na nivou trening/test granice - ako bismo LOO medijanu računali na celom skupu, Stream test pesama bi i dalje ulazio u izračunavanje feature-a za trening pesme istog izvođača (i obrnuto), jer LOO samo izuzima trenutni red, ne i sve redove van trening skupa. Zato LOO medijanu računamo **isključivo unutar trening skupa** (koristeći samo ostale trening pesme istog izvođača), dok test skup dobija pun (ne-LOO) medijan Stream-a tog izvođača, računat isključivo na trening podacima - test-ova sopstvena vrednost Stream-a se nigde ne koristi za računanje njegovog sopstvenog feature-a.
...
Spearmanova korelacija (izračunata na trening skupu) potvrđuje umerenu do jaku pozitivnu vezu između dodate kolone i Stream-a, što znači da je ova kolona definitivno kandidat za prediktor.
...
Views, Likes i Comments su međusobno jako korelisani (multikolinearnost). Views zadržavamo kao glavnog predstavnika YouTube popularnosti (najjača veza sa Stream-om), dok od Likes i Comments možemo da napravimo rate feature-e umesto sirovih brojeva, što nam daje informaciju o tome koliko je publika angažovana prilikom gledanja videa, a ne samo koliko je video pogledan.

Ova transformacija je red-po-red (koristi samo kolone iz istog reda), pa ne nosi rizik od curenja - primenjujemo je identično na trening i na test.
...
I ova transformacija je red-po-red, pa je primenjujemo na oba skupa, a dijagnostički test (kruskal_effsize) radimo samo na trening skupu, kako ne bismo birali feature-e gledajući u test.

```{r}
dodaj_mood_kvadrant <- function(df) {
  df %>%
    mutate(
...
train <- dodaj_mood_kvadrant(train)
test <- dodaj_mood_kvadrant(test)

kruskal_effsize(train, Stream ~ Mood_kvadrant)
```
...
```{r}
train <- train %>% mutate(Key_faktor = factor(Key))
test <- test %>% mutate(Key_faktor = factor(Key))

table(train$Key_faktor)
```

```{r}
kruskal.test(Stream ~ Key_faktor, data = train)
kruskal_effsize(train, Stream ~ Key_faktor)
```

```{r}
ggplot(train, aes(x = Key_faktor, y = log1p(Stream), fill = Key_faktor)) +
...
Brišemo linkove i kolone bez prediktivne vrednosti za modele, kao i kolone koje su višak zbog feature engeneering-a, ili su nastale feature engineering-om, a nisu se pokazale kao da bi imale veći potencijal od inicijalnih verzija kolona. Ovaj korak primenjujemo identično i na trening i na test skup - podela na trening i test je urađena znatno ranije (pre imputacije i enkodiranja), pa ovde više ne generišemo novu podelu, samo završavamo pripremu kolona za modele.

Pored toga, sklonićemo kolone za koje je koeficijent korelacije ekstremno nizak i koje sigurno neće imati uticaj na ciljnu promenljivu:
...
- Mood_kvadrant, Is_live, Is_instrumental, Is_spoken, Is_collab - kolone nastale feature engineering, koje se nisu pokazale kao dobri prediktori ni nakon transformacija.

```{r}
drop_kolone <- function(df) {
  df %>%
    select(-X, -Url_spotify, -Uri, -Url_youtube, -Title, -Description, -Channel, -Album, -Key,
           -Mood_kvadrant, -Is_collab, -Is_spoken, -Is_instrumental, -Is_live,
           -Licensed, -official_video, -Likes, -Comments, -Key_faktor)
}
...
formula_ml <- as.formula(
  log1p(Stream) ~ Album_type + Loudness + Acousticness + Instrumentalness +
    Views + Artist_LOO_stream + Likes_rate + Comments_rate + Energy + Duration_ms,
    + Tempo + Liveness + Danceability + Valence +
...
Random Forest nema early stopping, pa možemo da ga treniramo direktno nad celim skupom za trening.
...
Pored broja iteracija, model zavisi i od hiperparametara poput maksimalne dubine stabla (max_depth) i stope učenja (eta), koji se ne mogu odrediti automatski, pa ćemo sprovesti kros validaciju da bismo pronašli ove parametre.
...
```
Sva tri modela postižu sličan nivo tačnosti, pri čemu treba imati u vidu da su ovo sada rezultati nad zaista neviđenim test skupom - podela na trening i test je urađena pre bilo kakve imputacije ili enkodiranja koje koristi Stream, tako da ove metrike (za razliku od ranije verzije analize) ne sadrže optimizam koji dolazi od curenja podataka.

Važno je napomenuti da su sve prikazane metrike izračunate na log-transformisanoj skali ciljne promenljive, a ne na originalnoj skali stream-ova. Ovo je neophodno jer Stream ima veoma desno asimetričnu raspodelu sa ekstremnim vrednostima, ali se metrike ne mogu direktno protumačiti.
