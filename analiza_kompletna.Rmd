...
ggplot(top_songs_long, aes(x = Label, y = Vrednost, fill = Metrika)) +
  ...
  labs(title = "Top 15 pesama po broju stream-ova (Stream naspram Views)", x = "", y = "Broj (milijarde)", fill = "")
```
...
Umesto izbacivanja Likes i Comments kolona u linearnoj regresiji, možemo probati i PCA (Principal Component Analysis) nad ove tri kolone, kako bismo probali da iskoristimo svu njihovu informaciju bez uvođenja multikolinearnosti u linearne modele. Pošto su Views, Likes i Comments izrazito desno asimetrični, prvo ćemo odraditi logaritamsku transformaciju, a zatim standardizovati podatke, jer je PCA osetljiva na razlike u varijansi između promenljivih.
...
brisanje_kolona = function(df) {
  df %>%
    select(-X, -Url_spotify, -Uri, -Url_youtube, -Title, -Description, -Channel, -Album, -Key,-Licensed, -Official_video, -Is_live, -Is_collab, -Is_spoken, -Is_instrumental, -Likes, -Comments)
}
...
Views je, kao i Stream, izrazito desno asimetričan, pa ga radimo logaritamsku transformaciju.

```{r}
full_formula = as.formula(
  log1p(Stream) ~ Album_type + Danceability + Energy + Loudness + Speechiness +
    Acousticness + Instrumentalness + Liveness + Valence + Tempo + Duration_ms +
    log1p(Views) + Artist_LOO_stream + Likes_rate + Comments_rate +
...
**Artist_LOO_stream** ima naizgled zanemarljiv koeficijent, ali je to posledica velikog raspona same promenljive, kada je on pomnožen sa tipičnom vrednošću, efekat je zapravo jedan od najizraženijih u modelu.

**Comments_rate** pokazuje jak negativan koeficijent (-34.21, p < 0.001), u skladu sa negativnom korelacijom uočenom ranije.
...
Stepwise selekcija je izbacila `Danceability`, `Instrumentalness`, `Tempo`, `Mood_quadrant` i `Key_factor`, a to su sve prediktori koji su se i u bivarijantnoj analizi pokazali kao zanemarljivi. Adjusted R² je ostao identičan uz jednostavniji model, što je i cilj stepwise selekcije.
...
Obična linearna regresija na testnom skupu R² od oko 0.481, vrlo blizu R² na trening skupu čije je vrednost oko 0.483, što znači da model dobro generalizuje i da nema znakova overfitting-a.
...
Lasso daje skoro identičan rezultat kao obična regresija (oko R² = 0.480 u odnosu na 0.481), uz vrlo blagu regularizaciju, izbačena su samo 4 pojedinačna koeficijenta (Danceability, jedan nivo Mood_quadrant-a, dva nivoa Key_factor-a). 
...
Definisaćemo formulu za sva tri modela, sličnu formuli korišćenoj kod linearnih modela, s tim što ovde ne moramo da radimo logaritamsku transformaciju Views, jer ovde nemamo problem sa nelinearnim odnosima.

```{r}
formula_model = as.formula(log1p(Stream) ~ Album_type + Loudness + Acousticness + Instrumentalness +
...
## Dodatna analiza: uticaj uključivanja pesama bez YouTube spota
...
## Poređenje punog skupa ili samo Youtube

```{r}
rezultati_pun_vs_yt = bind_rows(
  rf_eval %>% mutate(Varijanta = "Samo YouTube"),
  rf_eval_full %>% mutate(Varijanta = "Pun skup"),
  xgb_eval %>% mutate(Varijanta = "Samo YouTube"),
  xgb_eval_full %>% mutate(Varijanta = "Pun skup"),
  lgb_eval %>% mutate(Varijanta = "Samo YouTube"),
  lgb_eval_full %>% mutate(Varijanta = "Pun skup")
) %>%
...
tibble(actual = expm1(y_test), predicted = expm1(lgb_pred)) %>%
  ...
  theme_minimal() +
  labs(
    title = "LightGBM - predviđene naspram stvarnih vrednosti (Stream)",
...
- Ukolikoko je prioritet **interpretabilnost**, tj. razumevanje *zašto* neka pesma ima više ili manje stream-ova, uz jasne, direktno tumačive koeficijente — linearna regresija (nakon stepwise selekcije) ostaje solidan izbor, uprkos nižoj tačnosti.
