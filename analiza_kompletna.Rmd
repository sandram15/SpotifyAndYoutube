...
formula_ml = as.formula(log1p(Stream) ~ Album_type + Loudness + Acousticness + Instrumentalness +
    ...
    + Tempo + Liveness + Danceability + Valence + Party_index + Licensed_video_combo + Has_youtube)
```
...
```
Analiza važnosti prediktora kod XGBoost-a pokazuje obrazac uočen i kod Random Forest-a, odnosno da su Views i Artist_LOO_stream ubedljivo najuticajniji prediktori, dok ostale varijable doprinose mnogo manje. Ovo sugeriše da je najveći deo prediktivne moći modela vezan za popularnost sadržaja na YouTube-u i istorijski uspeh izvođača, dok audio karakteristike pesme (danceability, energy, tempo i sl.) imaju sporedan, ali ipak statistički primetan uticaj.
